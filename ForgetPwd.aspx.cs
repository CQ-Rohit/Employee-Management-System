...
public partial class ForgetPwd : System.Web.UI.Page
{
    ...
    protected void SendPassword()
    {
        ...
        if (dr.Read())
        {
            ...
            string msg = "Your Login Credential are : %0a Emp ID : " + txtEmpid.Text + " password : " + pwd;
            b.SendSms("EMPTTS",mob,msg);
            
            updatePassword(pwd);  // update new generated password...
        }
        else
        {
            lblmsg.Text = "OTP no longer valid !";            
    ...
    protected void updatePassword(string newpwd)
    {
        SqlConnection cn = new SqlConnection(ConfigurationManager.AppSettings["EMP"]);
        SqlCommand cmd = new SqlCommand();
        cn.Open();

        cmd.CommandText = "update Registration set pwd=@pwd where username=@username";
